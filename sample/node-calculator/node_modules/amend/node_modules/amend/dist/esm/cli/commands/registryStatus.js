...
export function registryStatusCommand(program) {
    program
        .command('registry')
        ...
        if (recent.length > 0) {
            const keyW = 30;
            const msgW = 25;
            console.log('  ' + 'KEY'.padEnd(keyW) + 'MESSAGE'.padEnd(msgW) + 'FILE');
            for (const r of recent) {
                const shortKey = r.key.length > keyW
                    ? '…' + r.key.slice(-(keyW - 1))
                    : r.key;
                console.log('  ' + shortKey.padEnd(keyW) + r.message.padEnd(msgW) + `${r.filePath}:${r.line}`);
            }
        }
        if (total > 0) {
            ...
            console.log('\n  Analytics:');
            if (topClass)
                console.log(`    Most common type  : ${topClass[0]} (${topClass[1]}x)`);
            console.log(`    Last 24h          : ${last24h} errors`);
        }
    });
}
//# sourceMappingURL=registryStatus.js.map