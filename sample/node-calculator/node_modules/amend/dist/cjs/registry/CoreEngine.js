...
exports.errorClassFrom = errorClassFrom;
exports.locationFrom = locationFrom;
...
// ── Internal helpers ──────────────────────────────────────────────────
function b64uEnc(text) {
    return Buffer.from(text, 'utf8').toString('base64url');
}
function b64uDec(token) {
 ...
 */
function encode(filePath, line, className, funcName, message, errorClass) {
...
function decode(key) {
    ...
    if (parts.length !== 3) {
        throw new Error(`Invalid amend key (expected 3 dot-separated segments after prefix): ${key}`);
    }
